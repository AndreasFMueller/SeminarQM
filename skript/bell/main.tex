...
Wie bereits im Abschnitt~\ref{section:bell:epr} werden wir das Paper
von John Bell, h\"aufig als \enquote{Bell'sche Ungleichung} bezeichnet,
zuerst auf einfache Weise herleiten und danach mathematisch beweisen.

\subsection{Die Bell'sche Ungleichung\label{subsection:bell:bell:idee}}

\subsection{Mathematische Herleitung\label{subsection:bell:bell:herleitung}}

Die Vorhersage der Quantenmechanik verletzt  %TODO: bitte noch deutsch und deutlich
jedoch die Bell'sche Ungleichung.
Wir betrachten dazu erneut die Pauli Matrizen \eqref{equ:bell:paulimatrizen}
mit den Eigenvektoren $|{+}x\rangle$ und $|{-}x\rangle$.
Damit ist der Spin Zustand $|\psi\rangle$ wie in \eqref{equ:bell:psix}
hergeleitet
\[
    |\psi\rangle = \frac{1}{\sqrt{2}} \Big( 
...
Die Resultate der Messungen $A$ und $B$ h\"angen damit von $\lambda$ und 
den Einstellungen der Detektoren $a$ und $b$ ab und betragen,
da die Messungen dem Spin entsprechen, entweder ${+}1$ oder ${-}1$ bzw.
Spin up oder Spin down \textsuperscript{[Citation needed]}:
\begin{align}
    A(a,\lambda) = \operatorname{sign} \Big( \langle a,\lambda \rangle \Big) && 
    B(b,\lambda) = \operatorname{sign} \Big( \langle b,\lambda \rangle \Big) 
\end{align}
Der Fall $A(a,\lambda) = 0$ ist mit dieser Definition nat\"urlich undefiniert,
doch da die Wahrscheinlichkeit dieses Falls $0$ ist, kann dieser Fall hier
schlicht vernachl\"assigt werden.

Wir betrachten nun ein Partikel mit Spin Zustand $p$ mit der verborgenen
Variable $\lambda$, wobei $\lambda$ gleichverteilt auf
$\langle \lambda , p \rangle > 0$ ist.
Es sind also alle $\lambda$ m\"oglich, f\"ur welche die Messung von $p$
das Resultat $+1$ ergeben wird.

Die folgenden Beziehungen k\"onnen damit einfach hergeleitet werden:
\begin{align}
    E(a,a) &= -E(a,-a) = -1 \\
    E(a,b) &= 0 & \text{if } \langle a,b \rangle = 0
\end{align}


Unter der grundlegenden Annahme, dass die Detektoren $A$ und $B$ unabh\"angig
sind, kann die Korrelation $E$ zwischen der Resultate der Messungen $A$ und $B$
mit den Einstellungen $a$ und $b$ geschrieben werden als
\begin{equation}
    E(a,b) = \int A(a,\lambda) B(b,\lambda) \rho(\lambda) d\lambda
\end{equation}
mit der Wahrscheinlichkeitsdichte $\rho(\lambda)$.
