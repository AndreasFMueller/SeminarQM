...
% fixme evt. bessere Formulierung
\begin{equation}
\begin{aligned}
  \varepsilon H_1(x) &= \varepsilon a e_{-} x +b  \\
                     &= \varepsilon \frac{ U }{2 l} e_{-} x + 0  \\ 
  \Longrightarrow \qquad \varepsilon &= \frac{ U }{2 l} e_{-}; \, a = 1
\end{aligned}
\end{equation}
...
$\langle \psi_l^{(0)}|\, H_1 \,|\psi_k^{(0)}\rangle$ ergibt nur einen Beitrag wenn
\[
  k-l \mod 2 \equiv 1.
\]
...
Wollen wir ein Teilchen mit m\"oglichst grosser Wellenl\"ange, so m\"ussen wir die Masse $m$ 
resuzieren und die Geschwindigkeit $v$ m\"oglichst nah an $0$ halten.

Ganz $0$ darf die Geschwindigkeit nicht werden, da das Teilchen sonst die Energie
\[
  sin(0) = 0
\]
bek\"ame.

Daraus schliessen wir, dass das Universum am Anfang leer oder komplett ausgeglichen sein k\"onnte.
Durch diesen Zustand w\"aren dann alle Teile zum Stillstand gekommen.

Da dieser Zustand nicht stabil ist, hat sich in der 
Mitte\footnote{Annahme, das Universum ist dann endlich gross}
ein Teilchen (wieder\footnote{wieder, wenn man von einem sich wieder aufbauendem Universum ausgeht})
bewegt und die Energie so gross war, dass es eine Kettenreaktion ausgel\"osst
hat und so den Urknall gestartet hat.

Anders als eine klassische Explusion hat diese Explusion aber die benachbarten Wellen stimuliert
und so Teilchen aus dem ``Nichts'' (wieder) in das Universum gebracht.
