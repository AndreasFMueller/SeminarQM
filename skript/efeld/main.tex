...
\section{Grundlagen St"orungstheorie} %Fixme: alles
...
Ein grosser Vorteil dabei ist, dass f"ur die erste N"aherung der St"orung nur ein Term hinzuf"ugen werden muss.
Dieser kann ausserdem h"aufig aus dem Grundzustand abgeleitet werden.


%FIXME Erk\"arung zu den Formeln
\[
  E_c(\varepsilon) = E_n^{(0)} + \varepsilon^1 E_n^{(1} + \varepsilon^2 E_n^{(2)} + \ldots
\]
%FIXME: Welche Form hat $E_n$?

\[
  |\psi_n(\varepsilon)\rangle = |\psi_n^{(0)}(\varepsilon)\rangle + \varepsilon^1 |\psi_n^{(1)}(\varepsilon)\rangle + \varepsilon^2 |\psi_n^{(2)}(\varepsilon)\rangle + \ldots
\]

Wir betrachten in dieser Anwendung nur die erste N\"aherung und werten darum nur die Terme mit den 
Indizes $0$ und $1$ aus.
...
Potential\footnote{Das Feld ist die Ableitung des Potentials} 
definieren:
\begin{equation}
  H_1(x) = a x + b
\end{equation}
Die St"orung kann noch weiter vereinfacht werden:
% FIXME falsch: Weil die Barrieren unendlich hoch sind, k"onnen wir $b = 0$ setzen,
und da wir $a$ bereits durch $\varepsilon$ beschreiben, k\"onnen wir mit $a = 1$ vereinfachen.
$\varepsilon$ entspricht somit dem elektrischen Feld das wir anlegen.

% FIXME feld zu spannung korrekt?
$\varepsilon$ wird hier zur Berschreibung der St\"arke des elektrischen Feldes verwendet.
$\varepsilon$ ist definiert als die angelegte Spannung geteilt durch die Breite des Spalts
$\frac{ U }{2 l}$.
