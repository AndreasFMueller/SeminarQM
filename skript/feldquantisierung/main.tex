...
\rhead{Maxwell-Gleichungen und elektromagnetische Wellen}
...
\rhead{Vom Wellenfeld zu unabh"angigen Oszillatoren}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
	\draw[->,color=black,thick] (0, 0) -- +( 4, 0);
	\node[color=black] at (2.6, 0.2) {$k$};
	\draw[->,color=black,thick] (0, 0) -- +( 0, 2);
	\node[color=black] at (0.4,1.6) {$\varepsilon^{(1)}$};
	\draw[->,color=black,thick] (0, 0) -- +( -1.0, -1.0);
	\node[color=black] at (-0.25, -0.7) {$\varepsilon^{(2)}$};
	\draw[color=blue]   plot[smooth,domain=0:3.141] (4/3.141*\x,{1*cos(2*\x r)});
	\node[color=blue] at (1.0, 0.8) {$A$};
	\end{tikzpicture}
...
\rhead{Quantisierung des Wellenfeldes}
...
\rhead{Wechselwirkung zwischen Photonen und Elektronen}
