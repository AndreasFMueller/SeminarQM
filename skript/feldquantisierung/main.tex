...
Zudem gilt f"ur Transversal-wellen $\nabla \cdot A = 0$.
...
&= \frac{1}{\sqrt{V}} \sum_k \sum_{\alpha=1,2} (c_{k,\alpha}(0) \epsilon^{(\alpha)} e^{i (kx - \omega t)} + c^*_{k,\alpha}(0) \epsilon^{(\alpha)} e^{-i(kx - \omega t)})
\end{split}
\end{equation}
...
\begin{equation}
a|n\rangle = \sqrt{e_n}|n-1\rangle \quad a|2\rangle = \sqrt{2}|1\rangle  \quad a|1\rangle = 0|0\rangle \quad a|0\rangle = 0|0\rangle
\end{equation}
...
Der Eigenvektor von $N_{k,\alpha}$ ist der Zustandsvektor f"ur einen Zustand mit einer bestimmten Anzahl Photonen im Zustand $(k,\alpha)$. F"ur einen Zustand mit vielen Photonen mit verschiedenen Zust"anden bilden wir das diskrete Produkt aus Eigenvektoren:
...
\begin{equation}
\begin{split}
A(x,t) &= \frac{1}{\sqrt{V}} \sum_k \sum_{\alpha=1,2} \left(c \sqrt{\hbar/2 \omega_k}\right)\left[a_{k,\alpha}(0) e^{-i \omega_k t} \epsilon^{(\alpha)} e^{ikx} + a^+_{k,\alpha}(0) e^{i \omega_k t} \epsilon^{(\alpha)} e^{-ikx}\right]\\
...
Die Energieskala ist hier nun frei w"ahlbar. Durch die Wahl von $H|0\rangle = 0$ folgt:
...
Um die Hamilton-Funktion eines Teilchens im Magnetfeld zu erhalten, wird $p$ gem"ass \ref{section:hamilton-funktion-im-magnetfeld} durch $p -eA/c$ ersetzt. $A$ ist nun das quantisierte Feld. Die Hamilton-Funktion wird somit zu:
