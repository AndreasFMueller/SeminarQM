...
\end{align}
Die vier Maxwell-Gleichungen k"onnen also auch als zwei Gleichungen dargestellt werden.
...
\lambda = \frac{2 \pi}{|k|}
\end{align*}
Durch setzen von $c_{k,\alpha}(t) = c_{k,\alpha} e^{-i \omega_k t}$ sowie $u_{k,\alpha}(x) = \varepsilon^{(\alpha)} e^{ik \cdot x}$ kann die Gleichung auch in einen Zeit- und einen Ortsabh"angigen Teil aufgeteilt werden:
\begin{equation*}
...
Es stellt sich nun die Frage, wie die Vektoren $k$ und $\varepsilon^{(\alpha)}$ gew"ahlt werden m"ussen. Damit die Orthogonalit"at der Fourier-Reihe gew"ahrleistet ist, m"ussen $(\varepsilon^{(1)}, \varepsilon^{(2)} \text{ und } \varepsilon^{(3)})$ orthogonal sein. Wir w"ahlen die L"ange $1$. Durch Linearkombinationen k"onnen somit beliebige Polarisation gew"ahlt werden. Zudem setzen wir fest, dass $\varepsilon^{(3)}$ sowie $k$ parallel sind.

Die Berechnung von $B$ und $E$ liefert:
\begin{equation*}
...
\end{equation*}
Hier darf der Term $k \times \varepsilon^{(\alpha)}$ nicht verschwinden, damit sich eine Welle bilden kann. Somit f"allt $\varepsilon^{(3)}$ weg, da das Vektorprodukt von parallelen Vektoren null ist. Wir definieren somit, dass $(\varepsilon^{(1)}, \varepsilon^{(2)} , k/|k|)$ ein orthogonales Rechtssystem aus Einheitsvektoren bilden.
...
u^+_{k,\alpha}(x) &= \varepsilon^{(\alpha)} e^{-ik \cdot x}
\end{align*}
kann die Gleichung wiederum in einen Zeit- und einen Ortsabh"angigen Teil aufgeteilt werden:
\begin{equation*}
A(x,t) = \frac{1}{\sqrt{V}} \sum_k \sum_{\alpha=1,2} \sqrt{\frac{\hbar}{2 \varepsilon_0 \omega_k}}\left[a_{k,\alpha}(t) u_{k,\alpha}(x) + a^+_{k,\alpha}(t) u^+_{k,\alpha}(x) \right]
\end{equation*}
...
Der Spin von Photonen und Elektronen wird vernachl"assigt, da diese Einfluss zwar vorhanden, aber sehr klein ist.
...
\begin{itemize}
	\item die Wahrscheinlichkeit der stimulierten Absorption $B_{12} N_1 u$
	\item die Wahrscheinlichkeit der stimulierten Emission $B_{21} N_2 u$ und
	\item die Wahrscheinlichkeit der spontanen Emission $A_{21} N_2$ (F"ur $u=0$)
\end{itemize}
gegeben ist.
