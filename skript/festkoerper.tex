...
\begin{equation*}
\sum_{k'\in\Gamma'}\sum_{k''\in\Gamma'}V_{k''}u_{k'}e^{i(k+k'+k'')x}
=
\sum_{\bar k\in\Gamma'}\sum_{k''\in\Gamma'}V_{\bar k-k''}u_{k''}e^{i(k+\bar k)x}
\end{equation*}
Dabei haben wir $\bar k=k'+k''$ gesetzt, $k'=\bar k-k''$. Schreiben wir jetzt
wieder $k'=\bar k$, dann k"onnen wir die
...
0
\end{align*}
Durch Koeffizientenvergleich k"onnen wir jetzt ablesen, dass
\begin{equation}
\biggl(
\frac{\hbar^2}{2m_e}(k+k')^2
-E(k)
...
wird.
Diese Situation kann eintreten, wenn $k\simeq -\pi/a$ und $k'\simeq 2\pi/a$,
also in der N"ahe der vertikalen Linien in Abbildung~\ref{skript:dispersion}.
