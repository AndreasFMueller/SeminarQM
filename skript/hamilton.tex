...
\begin{satz}[Maupertuis]
Unter allen m"oglichen Pfaden $q(t)$ von $q_0 = q(t_0)$ nach $q_1=q(t_1)$
realisiert ein mechanisches System denjenigen, f"ur den
$W[q(t)]$ minimal ist.
\end{satz}
...
\end{align*}
Im dritten Schritt verwenden wir, dass $\eta(t_0)=\eta(t_1)=0$.
Die letzte Gleichung ist nur dann f"ur jede beliebige Funktion $\eta(t)$
...
weglassen.
Wir k"onnen jetzt die Euler-Gleichungen f"ur die Lagrange-Funktion
$L(x', x)= |x'|\,n(x)$ 
herleiten:
\begin{align*}
...
zu den Variablen $q_i$ die zus"atzlichen Variablen $v_i$ und die
...
\end{gather}
Die dritte Gleichung heisst auch die {\em Jacobi-Identit"at}.
\end{satz}
