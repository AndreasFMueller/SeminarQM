...
\begin{align*}
  &f(x \oplus s) = f(x)
\end{align*}
Eine Funktion mit dem Wert von $s = 0^n$ und $n = 3$ w"urde diese Eigenschaft
somit nicht erf"ullen.
\begin{figure}[H]
 \centering
...
Funktion gemacht werden muss: Nur 2--1 Funktionen sind von Intersse.
Ein Beispielfunktion ist etwa $s = 110$ : 
\begin{figure}[H]
 \centering
 \begin{tikzpicture}[ele/.style={fill=black,circle,minimum width=1pt,inner
     sep=1pt},every fit/.style={ellipse,draw,inner sep=-2pt}]
  ...
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa2) -- (bb1);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa3) -- (bb3);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa4) -- (bb3);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa5) -- (bb5);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa6) -- (bb5);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa7) -- (bb7);
  \draw[->,thick,shorten <=2pt,shorten >=2] (aa8) -- (bb7);
 \end{tikzpicture}
\end{figure}
...
\begin{tikzpicture}
  ...
  \draw[->] (0,0) -- (0,7) node[above] {$\mathcal{O}(log)$};
  ...
  \node [xshift=10cm,yshift=5.5cm] {$\mathcal{O}(\frac{n}{2} * \log 2)$};
  \node [xshift=10cm, yshift=1.7cm] {$\mathcal{O}(3 * \log n)$};
\end{tikzpicture}
\end{figure}
...
Somit wurde ein Algorithmus, dessn Laufzeit auf dem klassischen Computer bei sehr grossen
Zahlen schnell w"achst ( in diesem Fall ist das Wachstum mit $O(2^{n/2})$
angegeben ) auf eine L"osung mit einem viel kleineren Wachstum von $O(n^3)$
