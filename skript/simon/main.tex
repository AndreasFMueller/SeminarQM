...
Funktion $f$ gemacht werden muss: Nur 2--1 Funktionen, daher f"ur jedes $f(x)$
gibt es genau zwei $x$, sind von Interesse. Ein m"ogliches Beispiel mit
Periode $s = 110$ ist:
\begin{figure}[H]
 \centering
 \begin{tikzpicture}[ele/.style={fill=black,circle,minimum width=1pt,inner
 ...
 \end{tikzpicture}
\end{figure}
Die Frage ist nun, wenn eine Blackbox Funktion $f$ gegeben ist, was ist die
Periode $s$. 
\section{L"osung}
...
Jedoch m"uss f"ur alle Werte "uberpr"uft werden, ob die Bedingung der
