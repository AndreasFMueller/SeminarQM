...
Periodizität.
Denn, die eine Funktion mit dem Wert von $s = 0^n$ und $n = 3$ erfüllt die
Eigenschaft nicht.
\begin{figure}[H]
 \centering
 ...
 \end{tikzpicture}
\end{figure}
Des weiteren müssen folgende Einschränkungen bezüglich der Blackbox Funktion gemacht werden: 
Nur 2--1 Funktionen sind von Interssen. 
...
Nun ist aber nicht klar, dass diese Funktion periodisch ist. Für jeden Wert
muss nun überprüft werden, ob die folgende Bedingung gilt:
\[
    f(x) = f(x \oplus s) 
\]
...
Jedoch müssen für alle Werte überpüfut werden, ob die Periodizität Bedingung
noch gilt.
